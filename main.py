  ...
  async def on_message(self, message):
    ...
    if message.content.startswith(PREFIX):
      ...
      if message.channel.id in read_ID:
        ...
        # 再生する音声の生成
        create_MP3(message.content, name, channelname, rolename)
...
class dictionaryCog(commands.Cog, name="辞書"):
  ...
  async def deleteword(self, ctx, arg):
    ...
    os.rename('/home/runner/sui-replit/temp.txt',
              '/home/runner/sui-replit/user_dic.txt')
  ...
  async def wordlist(self, ctx):
    ...
    with open('/home/runner/sui-replit/user_dic.txt', mode='r') as f:
      ...
      embed = discord.Embed(title="登録されている単語一覧", description=list, color=GREEN)
...
class secletCog(commands.Cog, name="管理者専用"):
  ...
  async def on_message(self, message):
    ...
    if message.content.startswith(PREFIX):
      ...
      pass  # 機能がオフの場合無視
    elif re.match(r"す[いぃ]", message.content) or "sui" in message.content:
      file = discord.File('/home/runner/sui-replit/sui.png',
                          filename='sui.png')
...
async def setup_hook():
  ...
  await bot.add_cog(secletCog(bot))
